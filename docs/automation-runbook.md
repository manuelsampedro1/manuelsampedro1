...
- Selected Work row-count freeze at the current 48-row post-saturation state unless another explicit growth rationale is recorded.
...
- `examples/profile-evidence-map.md` keeping the primary claim repos, including `agent-start-gate` and `agent-output-contract`, linked so the profile can be reviewed by capability instead of repo count.
