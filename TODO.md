...
- Pause new proof-repo creation by default while the profile has 48 Selected Work rows and profile-proof audit holds at `100/100`; `agent-start-gate` and `agent-output-contract` were explicit post-saturation exceptions for materially new workflow gaps, and the next useful work should be curation, cross-repo demonstrations, hardening, or another separately justified gap.
...
- Keep `Selected Work` at the current 48-row post-saturation state unless a future change records another explicit growth rationale.
