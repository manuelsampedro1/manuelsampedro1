# TODO

- Set pinned repos on GitHub to `codex-review-packet`, `verify-by-change`, `briefboard-local`, and `manuelsampedro1`.
- Align `scripts/commit_daily_update.sh` with `docs/automation-runbook.md` so maintenance either refreshes root README latest links or stops claiming that it does.
- Run a public-view audit after pinned repos and profile metadata finish propagating.
- Add one lightweight real screenshot or SVG preview for `briefboard-local` if it improves the README without making it noisy.
- Publish one follow-up lab note or recipe that comes directly from each flagship repo, not from profile maintenance itself.
- Let the weekly quality audit run once under the new client-credibility rules, then tighten anything that still feels too meta.
