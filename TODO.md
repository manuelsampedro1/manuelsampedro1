...
- Pause new proof-repo creation by default while the profile has 49 Selected Work rows and profile-proof audit holds at `100/100`; `agent-start-gate`, `agent-output-contract`, and `agent-evidence-chain` were explicit post-saturation exceptions for materially new workflow gaps, and the next useful work should be curation, cross-repo demonstrations, hardening, or another separately justified gap.
...
- Keep `Selected Work` at the current 49-row post-saturation state unless a future change records another explicit growth rationale.
