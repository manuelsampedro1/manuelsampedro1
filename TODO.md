...
- Apply `recipes/real-diff-to-recipe.md` to future behavior-changing flagship repo diffs; `codex-review-packet` verification checklist packets, `agent-run-ledger` checklist import, `verify-by-change` empty-diff guard, `codex-review-packet` bounded diff packets, `agent-run-ledger` JSON doctor output, `verify-by-change` working-tree coverage, `briefboard-local` JSON restore, and `repo-flightcheck` dirty-tree detection are current quality bars.
